...
- Distillation-методы (2026-05-14): добавлены три новых метода в `src/text_distillation/`:
  - `vanilla_lm.py::distill_vanilla_lm` — class-conditional GPT-2 distillation по схеме `DiLM-main/src/generator.py` + `trainer_lm.py`. Single shared generator с per-label `<bos_y>` токенами и `<sep>` для пар. Fine-tune на real text → sample `k_per_class` per label. Зарегистрирован `@register_selection("vanilla_lm")`. Работает на `sst2`/`qqp`/`mnli-m` (paper scope).
  - `dilm.py::distill_dilm` — loader готовых синтетических датасетов из `DiLM-main/DiLM-synthetic-data/` (JSONL). DPC ∈ {5, 10, 20}, по 20 dataset_index'ов на пару (task, DPC). Никакого обучения генератора не требует — данные уже сгенерированы авторами paper. Зарегистрирован `@register_selection("dilm")`.
